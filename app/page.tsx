    ...
    <div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col lg:flex-row lg:items-end gap-6 mb-10">
            <div className="lg:w-3/5">
              ...
              <h2 className="text-4xl sm:text-5xl font-semibold tracking-[-0.06em] text-[#141821] leading-tight">Recent searches.</h2>
            </div>
            <div className="lg:w-2/5 lg:pb-1">
              <p className="text-lg font-normal leading-8 text-[#5B25C1]">Jump back into a search right where you left it.</p>
            </div>
          </div>
          <div className="flex gap-4 overflow-x-auto pb-2">
            {recentSearches.map((search, i) => (
              <div key={i} className="flex-shrink-0 bg-white rounded-2xl shadow-sm border border-[#ede9f6] px-6 py-5 min-w-[240px] cursor-pointer hover:shadow-md transition-shadow duration-200">
      ...
      <section className="ml-16 py-20 px-8 lg:px-16 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative rounded-3xl overflow-hidden p-8 lg:p-10" style={{ background: 'linear-gradient(135deg, #EA580C 0%, #1D4ED8 52%, #7629a5 100%)' }}>
            ...
            <div className="relative grid grid-cols-1 md:grid-cols-3 gap-5">
              {infoCards.map((card, i) => {
                ...
                return (
                  <div key={i} className="bg-white/30 backdrop-blur-md border border-white/30 rounded-2xl p-7 flex flex-col items-center gap-6 hover:bg-white/35 transition-colors duration-300">
                    <div className="w-[72px] h-[72px] rounded-2xl flex items-center justify-center" style={{ background: card.gradient }}>
                    ...
                    <Link href={card.ctaHref} className="inline-flex items-center gap-2 text-sm font-semibold text-white/90 hover:text-white transition-colors">
                      {card.cta}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col lg:flex-row lg:items-end gap-6">
            <div className="lg:w-3/5">
              ...
              <h2 className="text-4xl sm:text-6xl font-semibold tracking-[-0.06em] text-[#141821] leading-tight">Recommended<br />for you.</h2>
            </div>
            <div className="lg:w-2/5 lg:pb-2">
              <p className="text-lg font-normal leading-8 text-[#5B25C1]">10 nearby properties matched to your budget, preferred dates, and number of guests.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="ml-16 py-12 px-8 lg:px-16" style={{ background: '#F5F3EF' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-10 text-center">
            <Link href="/rentals/short-term" className="inline-flex items-center gap-2 px-8 py-4 rounded-full border-2 border-[#5b25c1] text-[#5b25c1] font-semibold hover:bg-[#5b25c1] hover:text-white transition-all duration-300">
              View all properties
              <ArrowRight size={16} strokeWidth={2} />
            </Link>
          </div>
      ...
      <div className="ml-16 pt-20 pb-14 px-8 lg:px-16 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <h2 className="text-4xl sm:text-6xl font-semibold tracking-[-0.06em] text-[#141821] mb-4">Find your next home.</h2>
          <p className="text-lg font-normal leading-8 text-[#5B25C1]">Long-term rentals for monthly stays, relocations, and extended travel.</p>
        </div>
      </div>
      ...
      <section className="ml-16 py-12 px-8 lg:px-16" style={{ background: '#F5F3EF' }}>
        <div className="max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-4 gap-5">
          {loading
            ? [...Array(4)].map((_, i) => <div key={i} className="animate-pulse"><div className="h-80 rounded-2xl bg-gray-200" /></div>)
            : longTermProperties.slice(0, 4).map((property) => <LongTermCard key={property.id} property={property} />)
          }
        </div>
      </section>
    </div>
