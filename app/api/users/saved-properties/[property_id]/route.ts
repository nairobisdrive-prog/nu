...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ property_id: string }> }) {
  try {
    ...
    await pool.execute('DELETE FROM saved_properties WHERE user_id = ? AND property_id = ?', [userRows[0].id, property_id]);
