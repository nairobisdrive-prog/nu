...
export default function LoginPage() {
  ...
  const { loginWithEmail, loginWithGoogle, register, error: authError, clearError } = useAuth();
  const router = useRouter();
  const error = localError || authError;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLocalError(''); clearError(); setLoading(true);
    try {
      if (isSignUp) {
        if (!displayName.trim()) { setLocalError('Please enter your name'); setLoading(false); return; }
  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] flex items-center justify-center py-12 px-4 pt-24">
      <div className="w-full max-w-md">
        <Link href="/" className="inline-flex items-center gap-2 text-[#756791] hover:text-[#141821] transition-colors mb-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl shadow-lg p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-[#141821]">{isSignUp ? 'Create Account' : 'Welcome Back'}</h1>
            <p className="text-[#756791] mt-2">{isSignUp ? 'Sign up to start renting' : 'Sign in to your account'}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {isSignUp && (
              <div>
                ...
                <input type="text" value={displayName} onChange={e => setDisplayName(e.target.value)} placeholder="John Doe" required={isSignUp}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#756791]">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm"><span className="px-4 bg-white text-[#756791]">or continue with</span></div>
          </div>

          <button type="button" onClick={handleGoogleSignIn} disabled={loading}
            className="w-full py-3 rounded-xl border border-gray-200 flex items-center justify-center gap-3 hover:bg-gray-50 transition-colors disabled:opacity-50">
          ...
          <p className="text-center mt-6 text-sm text-[#756791]">
            ...
            <button type="button" onClick={() => { setIsSignUp(!isSignUp); setLocalError(''); clearError(); }} className="text-[#5B25C1] font-medium hover:underline">
