...
function LongTermRentalsContent() {
  ...
  const [filters, setFilters] = useState({
    ...
    bedrooms: '', bathrooms: '', sort: 'newest',
  });
  ...
  useEffect(() => {
    async function fetchProperties() {
      setLoading(true);
      try {
        const params: Record<string, string> = { price_type: 'long_term', limit: '60', sort: filters.sort };
  ...
  const clearFilters = () => {
    setFilters({ location: '', propertyType: '', minPrice: '', maxPrice: '', bedrooms: '', bathrooms: '', sort: 'newest' });
    router.push('/rentals/long-term');
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-white rounded-xl border border-gray-200 p-1 gap-1">
              {(['grid', 'map'] as const).map(v => (
                <button key={v} onClick={() => setView(v)} className={`p-2 rounded-lg transition-all ${view === v ? 'bg-[#221854] text-white' : 'text-[#756791] hover:bg-gray-100'}`}>
                  {v === 'grid'
            ...
            <div className="max-w-7xl mx-auto">
              <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  {[
                    { label: 'Property Type', key: 'propertyType', type: 'select', options: ['', 'Villa', 'Condo', 'House', 'Apartment'] },
                    { label: 'Min Price /mo', key: 'minPrice', type: 'number', placeholder: '$0' },
                    { label: 'Max Price /mo', key: 'maxPrice', type: 'number', placeholder: '$50,000' },
        ...
        <div className="px-6 lg:px-16 py-8">
          <div className="max-w-7xl mx-auto">
            {loading ? (
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                  {properties.map(p => <LongTermCard key={p.id} property={p} />)}
                </div>
        ...
        <div className="px-6 lg:px-16 py-8">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-[640px]">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 overflow-y-auto max-h-[640px] pr-2">
              {loading ? [...Array(4)].map((_, i) => <div key={i} className="animate-pulse"><div className="h-40 bg-gray-200 rounded-t-2xl" /></div>) :
                properties.slice(0, 12).map((p, i) => (
                  <div key={p.id} className="bg-white rounded-2xl shadow-md overflow-hidden cursor-pointer hover:shadow-xl transition-shadow">
                    ...
                    <div className="p-3"><h3 className="text-sm font-semibold text-[#141821] mb-0.5">{p.title}</h3><p className="text-xs text-[#756791]">{p.city}, {p.state}</p><p className="text-sm font-semibold text-[#5B25C1]">${p.price.toLocaleString()}<span className="text-xs font-normal text-[#756791]">/mo</span></p></div>
                  </div>
                ))
              }
            </div>
